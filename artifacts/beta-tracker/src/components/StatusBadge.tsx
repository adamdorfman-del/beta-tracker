...
const BETA_STATUS: Record<BetaStatus, { label: string; className: string }> = {
  ...
  full:          { label: "Full",          className: "bg-teal-100 text-teal-700" },
...
export function BetaStatusBadge({ status }: { status: BetaStatus }) {
  const config = BETA_STATUS[status] ?? { label: status, className: "bg-gray-100 text-gray-600" };
  return <Badge {...config} />;
}

export function TesterStatusBadge({ status }: { status: TesterStatus }) {
  const config = TESTER_STATUS[status] ?? { label: status, className: "bg-gray-100 text-gray-600" };
  return <Badge {...config} />;
}

export function ApprovalStatusBadge({ status }: { status: ApprovalStatus }) {
  const config = APPROVAL_STATUS[status] ?? { label: status, className: "bg-gray-100 text-gray-600" };
  return <Badge {...config} />;
}

export function BatchStatusBadge({ status }: { status: BatchStatus }) {
  const config = BATCH_STATUS[status] ?? { label: status, className: "bg-gray-100 text-gray-600" };
  return <Badge {...config} />;
}
